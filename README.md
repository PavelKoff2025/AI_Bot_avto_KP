...
**Рабочее место менеджера отдела продаж** компании «Дом-Мастер»: от транскрибации звонка до комплекта КП.

Веб-CRM, проверка протокола по эталону, генерация коммерческих предложений, архитектурный и инженерный пакеты, утверждение и отправка клиенту (Email / Telegram). Telegram-бот — канал, не весь продукт.

> **About (для GitHub):** OfferDesk — CRM и генерация КП для «Дом-Мастер»: транскрибация → эталон → PDF → Email/Telegram. Docker · Go API · OpenAPI.
...
После звонка менеджер обычно вручную собирает смету, КП и приложения. OfferDesk берёт **текст транскрибации**, сверяет с эталоном, ведёт сделку в CRM и генерирует **готовые документы** через OpenAI + Jinja2 + WeasyPrint.

Основной контур — веб-CRM. В Telegram менеджер может пройти тот же сценарий в чате; клиенту КП уходит по Email или в привязанный чат.
...
- если данных мало — система просит дополнить транскрибацию и указывает, чего не хватает.

### 3. Выбор варианта коммерческого предложения
Три варианта тёплого контура:

| Вариант КП | Содержание |
|---|---|
...
В репозитории лежит `sample_dialog.txt` — протокол переговоров (строительство дома). Им удобно тестировать OfferDesk: CRM, бот и CLI.
